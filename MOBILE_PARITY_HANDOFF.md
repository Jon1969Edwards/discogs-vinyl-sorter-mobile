...
3b. **Genre edits:** AsyncStorage `spindle_genre_overrides` (Windows `genre_overrides.json`). Album detail **Edit genre**; survives Discogs refresh. Both apps write `spindle-genre:` onto the Discogs collection Notes field so the other device picks the edit up on the next collection load.
